...
print("Программа считывает многочлены из 2х файлов и фформирует файл, содержащий их сумму.")
...
while p >= 1:
    ...
    with open("d:/Мои документы/Алена/Обучение/8_4Python/Seminar4_#5/file"+ str(p) + ".txt", "w") as fl:
        fl.write(PolyN)
        print(f"Многочлен записан в файл {fl.name}")
    p-=1

with open("d:/Мои документы/Алена/Обучение/8_4Python/Seminar4_#5/file1.txt", "r") as fl:
    poly1 = fl.read()
    polyLst1 = poly1.replace(" = 0","").split(" + ")
with open("d:/Мои документы/Алена/Обучение/8_4Python/Seminar4_#5/file2.txt", "r") as fl:
    poly2 = fl.read()
...
with open("d:/Мои документы/Алена/Обучение/8_4Python/Seminar4_#5/fileS.txt", "w") as fl:
    fl.write(sumPoly)
